...
class rmClass {
    ...
    */
    public static function filltable($rowt) {
        include("../config.php");
        $successt = false;
        try{
    ...
    public static function hrFillForm($rowt) {
        ...
        try{
            ...
            $sqlt = "SELECT r.so_number,
                        ...
                        FROM rmemform AS r
                        WHERE r.user_id = $row
                        AND r.status = 'WAITING FOR SO_NUM'
                        GROUP BY r.RM_ID
    ...
    * @return Array that is used to fill in empty RM_Form
    */
    public static function hrPastFillForm($rowt) {
        //include("../config.php");
        $successt = false;
        try{
            $conn = new PDO(DB_DSN, DB_USERNAME, DB_PASSWORD);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $sqlt = "SELECT * FROM rmemform WHERE status = 'WAITING FOR VENDOR RESPONSE' or status = 'REJECTED'";
            $stmtt = $conn->prepare($sqlt);
            $stmtt->execute();
            $rowt = $stmtt->fetchAll(PDO::FETCH_NUM&PDO::FETCH_ASSOC);
            return $rowt;
    ...
    public static function vendFillForm($rowt) {
        ...
        try{
            ...
            $sqlt = "SELECT r.so_number,
                        ...
                        FROM rmemform AS r
                        WHERE r.user_id = $row
                        AND r.status = 'WAITING FOR VENDOR RESPONSE'
                        OR r.status = 'APPLICATION RECEIVED' OR r.status = 'SO_NUM APPLIED'
                        GROUP BY r.RM_ID
                        ORDER BY r.date_submitted DESC";
            $stmtt = $conn->prepare($sqlt);
            $stmtt->execute();
    ...
    */
    public function processForm() {
        ...
        try {
            ...
            $stmt2->execute();
            $rowt = $stmt2->fetchAll(PDO::FETCH_NUM&PDO::FETCH_ASSOC);
            ...
            recommended_hiring, notes, comments, status, user_id) VALUES (:ptitle, :sloc, :dsub, :numres,
            :pstart, :TMFP, :type, :rsdate, :rendate, :proj_client,
            :conf_perc, :hir_manag, :sen_manag, :engag_manag, :pcode,
            :t_salary, :rcc_level, :posit_desc, :rec_hire, :notes, :comments, :status, :user_id)";
